...
{	
	...
	public void rotate1()
	{
		xTop -= 10;xRight-=10;yLeft -=10;
	}
	
	public void rotate2()
	{
		xRight = xTop + 10;yRight = yTop - 10;
		//xTop = xRight - 10; yTop = yRight-10;
		xLeft = xTop + 10; yLeft = yTop + 10;
	}
	...
	{	
		try 
		{
			...
			for(int i=0;i<1;i++)
			{
				...
				updateRobots(); drawUpdatedRobot();Thread.sleep(100);eraseRobots();
				if(!withinObs(hg.obs1_startX,hg.obs1_startY,hg.obs1_width,hg.obs1_height) && !withinObs(hg.obs2_startX,hg.obs2_startY,hg.obs2_width,hg.obs2_height))
				{
					...
					else 
					{
						...
						else//(xfp == 1290) // fall point on right edge, reference point: right robot
						{
							...
							if(rotateFlag == 1)
							{
								rotate1();drawUpdatedRobot();
								Thread.sleep(200);eraseRobots();
								rotate1();drawUpdatedRobot();
								Thread.sleep(2000);eraseRobots();
								hval("rotate");
								//drawCSpace();
							}
							update(this.getGraphics());
							...
							/*if(xLeft == 990 && yLeft == 310)
							{
								//System.out.println("adfadfadfa");
								for(int x = 990; x!=50;x-=10)
								{
									eraseRobots();
									xTop-=10;
									rotate2();drawUpdatedRobot();update(getGraphics());
									Thread.sleep(100);
								}
								for(int y = 310; y!=610;y+=10)
								{
									eraseRobots();
									yTop+=10;rotate2();drawUpdatedRobot();update(getGraphics());
									Thread.sleep(100);
								}
							}*/
							int cur = hvalue[xRight/10][yRight/10];
							while(cur != 10)
							{
								if(withinCanvas(xRight, yRight,1))
								{
									eraseRobots();
									System.out.print("\ncur="+cur);
									System.out.print("\tleft="+hvalue[xRight/10-1][yRight/10]);
									System.out.print("\tright="+hvalue[xRight/10+1][yRight/10]);
									System.out.print("\tdown="+hvalue[xRight/10][yRight/10+1]);
									System.out.print("\tup="+hvalue[xRight/10][yRight/10-1]);
									...
									if(rotateFlag == 1) 
										...
										updateRobots();
									drawUpdatedRobot();Thread.sleep(50);
									cur -= 10;
								}
								else break;
							}
