...
public class hValGenerator 
{
	...
	int count=0, preCheck = 0,rotation = 0;
	int obs1_startX = 30, obs1_startY = 400, obs1_width=10, obs1_height=280;
	int presetArea = (obs1_width*obs1_height)/100;
	int obs2_startX = 60, obs2_startY = 400, obs2_width=100, obs2_height=280;
	...
	public hValGenerator(int x, int y, String direction)
	{
		...
		case "right":
			...
			total = total - presetArea - presetArea2;preCheck = total;
		...
		case "top":
			...
			break;
		case "rotate":
			rotateCSpace(obs1_startX,obs1_startY,obs1_width,obs1_height);
			presetArea = count;
			rotateCSpace(obs2_startX,obs2_startY,obs2_width,obs2_height);
			presetArea2 = count;
			total = total - presetArea - presetArea2;preCheck = total;
			break;
	...
	public void rotateCSpace(int obs_startX, int obs_startY,int obs_width, int obs_height)
	{
		//hval around obs
		count = 0;
		for(int i = obs_startX; i < (obs_startX+obs_width); i +=10)
		{
			count += 2;
			visited[i/10][obs_startY/10-2] = visited[i/10][obs_startY/10-1] = true;
			hval[i/10][obs_startY/10-2] = hval[i/10][obs_startY/10-1] = Integer.MAX_VALUE;
		}
		for(int i=obs_startY-10;i<obs_startY+obs_height;i+=10)
		{
			count ++ ;
			visited[(obs_startX+obs_width)/10][i/10]=true;
			hval[(obs_startX+obs_width)/10][i/10] = Integer.MAX_VALUE;
		}
		count += 2;
		visited[(obs_startX+obs_width)/10+1][(obs_startY+obs_height)/10-2] = visited[(obs_startX+obs_width)/10+1][(obs_startY+obs_height)/10-1]  = true;
		hval[(obs_startX+obs_width)/10+1][(obs_startY+obs_height)/10-2] = hval[(obs_startX+obs_width)/10+1][(obs_startY+obs_height)/10-1]  = Integer.MAX_VALUE;
	}
